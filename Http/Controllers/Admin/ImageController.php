...
class ImageController extends AdminBaseController
{
    ...
    public function store($villa_id,Request $request)
    {
        ...
        if($villa_id != 0){
            ...
            $villa->images()->attach([$savedFile->id => ['order' => $villa->images->count()+1]]);
    ...
    public function thumbnail($villa_id,$image_id,Image $image)
    {
        ...
        return redirect()->back()
            ->withSuccess(trans('core::core.messages.resource updated', ['name' => trans('villamanager::images.title.images')]));
    }
}
