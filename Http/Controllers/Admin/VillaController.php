...
Status $status)
    {
        ...
        $this->assetManager->addAssets([
            ...
            'villa.js' => Module::asset('villamanager:js/main.js'),
            'select2.js' => Module::asset('villamanager:js/select2.js'),
        ]);

        $this->assetPipeline->requireJs('bootstrap-datepicker.js');
        $this->assetPipeline->requireJs('select2.js');
        $this->assetPipeline->requireCss('bootstrap-datepicker.css');
        $this->assetPipeline->requireCss('villa.css');
        $this->assetPipeline->requireJs('villa.js');
    ...
    public function store(StoreVillaRequest $request)
    {
        ...
        if ($request->get('button') === 'index') {
            return redirect()->route('admin.villamanager.villa.index')
                ->withSuccess(trans('core::core.messages.resource created', ['name' => trans('villamanager::villas.title.villas')]));
        }

        if($request->get('copy')){
            return redirect()->route('admin.villamanager.villa.edit',$villa->id)
                ->withSuccess(trans('core::core.messages.resource created', ['name' => trans('villamanager::villas.title.villas')]));
        }
     ...
     * @return Response
     */
    public function copy(Villa $villa)
    {
        $facilities = $this->facilities->all();
        $areas = $this->area->all();
        $categories = $this->category->all();
        $statuses = $this->status->lists();
        return view('villamanager::admin.villas.copy', compact('facilities','villa','areas','categories','statuses'));
    }
    ...
    public function update(Villa $villa, UpdateVillaRequest $request)
    {
        ...
        if ($request->get('button') === 'index') {
            return redirect()->route('admin.villamanager.villa.index')
                ->withSuccess(trans('core::core.messages.resource updated', ['name' => trans('villamanager::villas.title.villas')]));
        }
        return redirect()->back()
            ->withSuccess(trans('core::core.messages.resource updated', ['name' => trans('villamanager::villas.title.villas')]));
    }
    ...
    public function destroy(Villa $villa)
    {
        ...
        return redirect()->route('admin.villamanager.villa.index')
            ->withSuccess(trans('core::core.messages.resource deleted', ['name' => trans('villamanager::villas.title.villas')]));
    }
}
