...
class RateController extends AdminBaseController
{
    ...
    public function store($villa_id,Request $request)
    {
        if($request->get('id') != '' && $request->get('id') != 'new_rate'){
            ...
            $message = (trans('core::core.messages.resource updated', ['name' => trans('villamanager::rates.title.rates')]));

        }else{
            $this->rate->create($request->all());

            $message = (trans('core::core.messages.resource created', ['name' => trans('villamanager::rates.title.rates')]));
    ...
    public function update(Rate $rate, Request $request)
    {
        ...
        return redirect()->route('admin.villamanager.rate.index')
            ->withSuccess(trans('core::core.messages.resource updated', ['name' => trans('villamanager::rates.title.rates')]));
    }
    ...
    public function destroy(Rate $rate,Request $request)
    {
        ...
        return redirect()->route('admin.villamanager.rate.index')
            ->withSuccess(trans('core::core.messages.resource deleted', ['name' => trans('villamanager::rates.title.rates')]));
    }
}
