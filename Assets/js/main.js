...
$( document ).ready(function() {
    ...
    $('#import-booking-form').on('submit',function () {
        var btn = this;
        if(!window.confirm('Are you sure the ics file is correct?'))
            return false;
    ...
    if($('#calendar').length){
        $('#calendar').fullCalendar({
            ...
            header: {
                left: 'prev,next',
                center: 'title',
                right: 'importButton,addButton,disableButtons,trashButton'
            },
    ...
    var publish_type = $('#publish_price_type').val();
    $('#change_publish_price_type').on('click',function () {
        console.log(publish_type);
        if(publish_type == 1)
        {
            $('#percent').hide();
            $('#nominal').show();
            $('#publish_price_type').val(2);
            $(this).html('[percent]')
        }
        else if(publish_type == 2)
        {
            $('#nominal').hide();
            $('#percent').show();
            $('#publish_price_type').val(1);
            $(this).html('[nominal]')
        }
        publish_type = $('#publish_price_type').val();
    });
    var agent_price_type = $('#agent_price_type').val();
    $('#change_agent_price_type').on('click',function () {
        if(agent_price_type == 1)
        {
            $('#agent_percent').hide();
            $('#agent_nominal').show();
            $('#agent_price_type').val(2);
            $(this).html('[percent]')
        }
        else if(agent_price_type == 2)
        {
            $('#agent_nominal').hide();
            $('#agent_percent').show();
            $('#agent_price_type').val(1);
            $(this).html('[nominal]')
        }
        agent_price_type = $('#agent_price_type').val();
    });
});
