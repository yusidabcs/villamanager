...
namespace Modules\Villamanager\Sidebar;
    ...
    public function extendWith(Menu $menu)
    {
        $menu->group(trans('core::sidebar.content'), function (Group $group) {
            $group->item(trans('villamanager::villas.title.main'), function (Item $item) {
                ...
                $item->item(trans('villamanager::categories.title.categories'), function (Item $item) {
                    $item->icon('fa fa-tag');
                    $item->weight(2);
                    $item->append('admin.villamanager.category.index');
                    $item->route('admin.villamanager.category.index');
                    $item->authorize(
                        $this->auth->hasAccess('villamanager.categories.index')
                    );
                });

                /*$item->item(trans('villamanager::categories.title.reports'), function (Item $item) {
                    $item->icon('fa fa-tag');
                    $item->weight(2);
                    $item->append('admin.villamanager.report.index');
                    $item->route('admin.villamanager.report.index');
                    $item->authorize(
                        $this->auth->hasAccess('villamanager.reports.index')
                    );
                });*/
// append
